...
# The following solutions also works
SELECT au_fname, au_lname, city 
FROM authors
...
SELECT au_fname, au_lname, city 
FROM authors
WHERE city = "Oakland" AND au_fname = "Dean" OR city = "Berkeley" AND au_fname = "Dean";
...
# Also works:
SELECT COUNT(*) as 'Authors_per_state', state
FROM authors
group by state;
...
# Calculate the average price per book type, per publisher, then count the number of books with that type, per publisher
# as long as the average price is >12

# Count of books per booktype per publisher
SELECT pub_id, type, COUNT(*) 
FROM titles
GROUP BY pub_id, type;

# Average price per booktype per publisher
SELECT pub_id, type, avg(price) 
FROM titles
GROUP BY pub_id, type;
# ORDER BY pub_id, type;

# We return the count per publisher, per booktype but we first calculate the avg per pub and type and check its >12
SELECT pub_id, type, COUNT(*) 
FROM titles
GROUP BY pub_id, type
HAVING AVG(price) > 12;
...
SELECT 
	CASE 
		...
		WHEN qty < 20 THEN 'low sales'
		ELSE 'medium sales'
	END AS sales_category_1, 
    SUM(qty)
FROM sales
GROUP BY sales_category_1;
...
GROUP BY sales_category_1
HAVING SUM(qty) > 100
ORDER BY sales_category_1 DESC;
